...
### フェーズ6 CI 検証（workflow_dispatch on fix-container, run 28842416525）— 2026-07-07

`|| true` を外した状態で CI ビルドを実行。amd64/arm64 ともに「Modify image」ステップ（`docker build`＝impure.sh 実行）で失敗。これは**修正が正しく顕在化させた本物の失敗**（旧 `|| true` が隠していたもの）。

`mise install` が non-zero で終了し、`set -e` で rsplug サブシェルが中断 → rsplug 未実行。失敗ツール:

- `npm:fish-lsp/gnhf/hunkdiff/trash-cli`（両arch）: `npm` not found → **node がインストールされていない**。
- `cargo:idevice_pair`（両arch）: cargo ビルド失敗（libimobiledevice 等のシステムライブラリ不足）。
- `cargo:ghzinga`（amd64のみ; arm64 は成功）: `ld returned 127`（mise の cargo ビルド時にリンカ `ld` が PATH に無い）。
- `github:neowright`（arm64のみ）: arm64-linux バイナリがリリースに存在しない。
- `pipx:aiocr`（両arch）: uv exit 2。

### node 未導入の根本原因 — 2026-07-07

mise 公式 JSON スキーマでは `os` フィルタの例は `["linux","macos","windows","macos/arm64","linux/x64"]` で、bare `"linux"` は有効（linux にマッチするはず）。しかし CI（mise **2026.6.11**）では `node = { version="latest", os=["macos","linux"] }` で node がスキップされた（成功/失敗リストのいずれにも載らず＝os フィルタで除外）。ホスト（2026.6.14）では macos 経由でマッチすることは実証済み。→ mise 2026.6.11 における配列 os フィルタの linux マッチングの不具合・実装差異と判断。

**修正**: `node` の os フィルタを削除（`node = "latest"`）し、フィルタ挙動に依存せず全プラットフォームで導入。これにより npm 系ツールが解決する。

### 対応方針（フェーズ6 第2反復）— 2026-07-07

- `mise install`（フルツールセット）は**ベストエフォート**（`|| echo` で失敗をログ化し継続）。一部ツールはコンテナ環境で原理上インストール不可（バイナリ不在・lib 不足）であり、nvim/シェル核心に影響しない。これは nvim 破壊を隠していた旧 `|| true` とは区別される（失敗は可視化、本質ステップは厳格）。
